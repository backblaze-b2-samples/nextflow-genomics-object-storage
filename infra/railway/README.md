...
| API | `B2_REGION`, `B2_BUCKET_NAME`, `B2_PUBLIC_URL_BASE`, `API_CORS_ORIGINS`, `API_CORS_ORIGIN_REGEX`, `ENABLE_DOCS`, `ALLOWED_KEY_PREFIX`, rate and size settings | Non-secret service configuration | The S3 endpoint is derived from `B2_REGION`. Keep values in Railway, not source; set exact production CORS origins and `ENABLE_DOCS=false`. |
