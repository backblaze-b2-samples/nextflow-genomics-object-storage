...
| `B2_REGION`, `B2_BUCKET_NAME`, `B2_PUBLIC_URL_BASE`, `ENABLE_DOCS`, `ALLOWED_KEY_PREFIX`, rate settings | Non-secret configuration | The S3 endpoint is derived from `B2_REGION`. Set `ENABLE_DOCS=false` in production. |
...
`project-name` and `repository-name` (both `nextflow-genomics-object-storage`, so the
